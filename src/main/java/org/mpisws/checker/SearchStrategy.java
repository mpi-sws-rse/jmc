...
public interface SearchStrategy {
   ...
   * map. It also checks for a deadlock between the threads in using the monitors. If a deadlock is
   * detected, the method sets the {@link RuntimeEnvironment#deadlockHappened} flag to true and the
   * {@link RuntimeEnvironment#executionFinished} flag to true. Otherwise, the method selects the
   * next thread to run.
   *
   ...
   * thread symbolic operations. Otherwise, it updates the path symbolic operations with the negated
   * symbolic operation.
   *
   * @param thread is the thread that is going to execute the symbolic operation.
   * @param symbolicOperation is the symbolic operation that is going to be executed.
   ...
   *
   * <p>It adds the symbolic operation to the {@link RuntimeEnvironment#pathSymbolicOperations} list
   * and the {@link RuntimeEnvironment#threadSymbolicOperation} map.
   *
   * @param symbolicOperation is the symbolic operation that is going to be executed.
   * @param thread is the thread that is going to execute the symbolic operation.
   */
  ...
  default Thread pickNextReadyThread() {
    ...
    if (!readyThread.get().isEmpty()) {
      ...
      if (computeUnblockedRecvThread()) {
        ...
        LOGGER.error(
            "There is a deadlock between the threads in executing blocking receive operations");
        printExecutionTrace();
  ...
  default boolean computeUnblockedRecvThread() {
    ...
    for (Map.Entry<Long, ReceiveEvent> entry : RuntimeEnvironment.blockedRecvThreadMap.entrySet()) {
      if (isMessageAvailable(entry.getValue())) {
        ...
        result = true;
        LOGGER.debug(
            "The thread {} is unblocked, since the message is available", jmcThread.getName());
      }
    }
    return result;
  }
   ...
   *
   * <p>This method checks whether the monitor is available or not. If the monitor is available, the
   * method removes the monitor request from the {@link RuntimeEnvironment#monitorRequest} map and
   ...
   * available, the method removes the join request from the {@link RuntimeEnvironment#joinRequest}
   * map and calls the {@link #nextJoinEvent} method. Otherwise, the method suspends the candidate
   * thread and selects another random thread to run.
   *
   * @param thread the candidate thread.
   * @return the candidate thread if it can run, otherwise selects another random thread.
  ...
  default Thread handleJoinRequest(Thread thread) {
    ...
    if (!RuntimeEnvironment.createdThreadList.contains(joinRes)
      ...
      LOGGER.debug(
          "Request of {} to join {} removed from the joinRequest. {} not in createdThreadList or"
              + " readyThread list",
          thread.getName(),
          joinRes.getName(),
          joinRes.getName());
   ...
   * RuntimeEnvironment#suspendedThreads} list. This action is required when the selected thread is
   * waiting for a monitor or a join request.
   *
   ...
   * RuntimeEnvironment#readyThread} list. This action is required when the monitor or join request
   * of the selected thread is available.
   *
   * @param thread the selected thread.
   */
   ...
   * RuntimeEnvironment#suspendedThreads} list whether the monitor request of the thread is equal to
   * the given monitor or not. If the monitor request of the thread is equal to the given monitor,
   * the thread is added to the list of suspended threads that are waiting for the monitor.
   *
   * @param monitor the monitor that the suspended threads are waiting for.
   ...
   *
   * <p>This method is used to detect potential deadlocks in the threads that are waiting to enter a
   * monitor. It computes the transitive closure of the (@monitorRequest \cup @monitorList) relation
   * and checks whether the relation is irreflexive or not. If the relation is irreflexive, there is
   * no deadlock and the method returns false. Otherwise, there is a deadlock and the method returns
   * true.
   *
