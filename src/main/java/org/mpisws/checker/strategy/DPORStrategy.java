...
public abstract class DPORStrategy implements SearchStrategy {
     ...
     *
     * <p>This method adds the event to the current graph. It sets the {@link #currentGraph} with
     * the given event.
     *
     ...
     * does not have any previous graphs. Then, it calls the {@link DPOR#visit(ExecutionGraph,
     * List)} method to visit the current graph with the given event.
     *
    ...
    protected void updateCurrentGraph(ThreadEvent threadEvent) {
        ...
        if (newGraphs.size() == 1) {
            ...
            System.out.println(
                    "[DPOR Strategy Message] : There is no new graph from model checker");
            System.out.println(
                    "[DPOR Strategy Message] : visited full execution graph G_" + numOfGraphs);
            ...
            //            System.out.println("[DPOR Strategy Debugging Message] : The COs of the
            // visited
            // graph are : ");
            //            currentGraph.getCOs().forEach();
            //            System.out.println("[DPOR Strategy Debugging Message] : The STs of the
            // visited
            // graph are : ");
            //            currentGraph.getSTs().forEach(System.out::println);
            //            System.out.println("[DPOR Strategy Debugging Message] : The JTs of the
            // visited
            // graph are : ");
            //            currentGraph.getJTs().forEach(System.out::println);
            //            System.out.println("[DPOR Strategy Debugging Message] : The TCs of the
            // visited
            // graph are : ");
            //            currentGraph.getTCs().forEach(System.out::println);
            //            System.out.println("[DPOR Strategy Debugging Message] : The events order
            // of the
            // visited graph are : ");
     ...
     * graph and checks whether the left hand side of the pair is equal to the given thread event.
     * If it is not equal, it sets the {@link #currentGraph} with the new graph. Otherwise, it
     * removes the new graph from the {@link #mcGraphs} list.
     *
    ...
    public void nextMainStartEvent(MainStartEvent mainStartEvent) {
        ...
        if (guidingActivate) {
            ...
            System.out.println(
                    "[DPOR Strategy Message] : The main start event is passed to the model"
                        + " checker");
    ...
    public void nextConAssumeRequest(ConAssumeEvent conAssumeEvent) {
        ...
        if (guidingActivate) {
            ...
            System.out.println(
                    "[DPOR Strategy Message] : The concrete assume event is passed to the model"
                        + " checker");
    ...
    public void nextAssumeBlockedRequest(AssumeBlockedEvent assumeBlockedEvent) {
        ...
        if (guidingActivate) {
            ...
            System.out.println(
                    "[DPOR Strategy Message] : The assume blocked event is passed to the model"
                        + " checker");
     ...
     * created {@link JoinEvent} is added to the {@link #currentGraph} if the {@link
     ...
     *
     * <p>This method represents the required strategy for the next finish event. It creates a
     * {@link FinishEvent} for the corresponding finishing execution request of a thread and records
     * it. The created {@link FinishEvent} is added to the {@link #currentGraph} if the {@link
     * #guidingActivate} is true. Otherwise, it passes the event to the {@link #dpor} model checker.
     ...
     * {@link #nextFinishEvent(Thread)} and picks the next thread based on the guiding execution
     * graph if the {@link #guidingActivate} is true. Otherwise, it picks the next random thread.
     *
     * @param thread is the thread that is going to be finished.
     * @return the next thread that is going to be executed.
     */
     ...
     *
     * <p>This method represents the required strategy for the next failure event. It creates a
     * {@link FailureEvent} for the corresponding failing execution request of a thread and records
     * it. The created {@link FailureEvent} is added to the {@link #currentGraph} if the {@link
     * #guidingActivate} is true. Otherwise, it passes the event to the {@link #dpor} model checker.
     *
     ...
     *
     * <p>This method saves the execution graphs to a file. It creates a new file with the given
     * file name and writes the execution graphs to the file.
     *
    ...
    protected Set<Pair<Event, Event>> findNewSTs() {
        ...
        for (Pair<Event, Event> st : guidingExecutionGraph.getSTs()) {
            ThreadEvent firstThreadEvent =
                    findThreadEventInCurrentGraph((ThreadEvent) st.component1());
            ThreadEvent secondThreadEvent =
                    findThreadEventInCurrentGraph((ThreadEvent) st.component2());
    ...
    protected Set<Pair<Event, Event>> findNewMCs() {
        ...
        for (Pair<Event, Event> mc : guidingExecutionGraph.getMCs()) {
            ThreadEvent firstThreadEvent =
                    findThreadEventInCurrentGraph((ThreadEvent) mc.component1());
            ThreadEvent secondThreadEvent =
                    findThreadEventInCurrentGraph((ThreadEvent) mc.component2());
            if (firstThreadEvent != null && secondThreadEvent != null) {
    ...
    protected Set<Pair<Event, Event>> findNewTCs() {
        ...
        for (Pair<Event, Event> tc : guidingExecutionGraph.getTCs()) {
            ThreadEvent firstThreadEvent =
                    findThreadEventInCurrentGraph((ThreadEvent) tc.component1());
            ThreadEvent secondThreadEvent =
                    findThreadEventInCurrentGraph((ThreadEvent) tc.component2());
            if (firstThreadEvent != null && secondThreadEvent != null) {
    ...
    protected Set<Pair<Event, Event>> findNewPCs() {
        ...
        for (Pair<Event, Event> pc : guidingExecutionGraph.getPCs()) {
            ThreadEvent firstThreadEvent =
                    findThreadEventInCurrentGraph((ThreadEvent) pc.component1());
            ThreadEvent secondThreadEvent =
                    findThreadEventInCurrentGraph((ThreadEvent) pc.component2());
            if (firstThreadEvent != null && secondThreadEvent != null) {
    ...
    protected Set<Pair<Event, Event>> findNewJTs() {
        ...
        for (Pair<Event, Event> jt : guidingExecutionGraph.getJTs()) {
            ThreadEvent firstThreadEvent =
                    findThreadEventInCurrentGraph((ThreadEvent) jt.component1());
            ThreadEvent secondThreadEvent =
                    findThreadEventInCurrentGraph((ThreadEvent) jt.component2());
            if (firstThreadEvent != null && secondThreadEvent != null) {
