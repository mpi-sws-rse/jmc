...
public abstract class DPORStrategy implements SearchStrategy {
  ...
  /**
   * @property {@link #mcGraphs} is used to store the execution graphs that are generated by the
   *     model checker.
   */
  ...
  /**
   * @property {@link #guidingActivate} is used to indicate whether the {@link
   *     #guidingExecutionGraph} is available or not.
   */
  ...
  /**
   * @property {@link #guidingNode} is used to store the current node of the {@link
   *     #guidingExecutionGraph}.
   */
  protected Node guidingNode;

  /**
   * @property {@link #guidingThread} is used to store the current active thread of the {@link
   *     #guidingExecutionGraph}.
   */
  protected int guidingThread;

  /**
   * @property {@link #guidingEvents} is used to store the events that are available in the {@link
   *     #guidingExecutionGraph}.
   */
  protected List<Event> guidingEvents;

  /**
   * @property {@link #guidingEvent} is used to store the current event of the {@link
   *     #guidingExecutionGraph}.
   */
  ...
  /**
   * @property {@link #executionGraphsPath} is used to store the path of the execution graphs
   *     directory.
   */
  ...
  public DPORStrategy() {
    ...
    if (!Files.exists(Paths.get(executionGraphsPath))) {
      System.out.println(
          "[DPOR Strategy Message] : Directory " + executionGraphsPath + " does not exist ");
      System.exit(0);
    }
    if (!Files.exists(Paths.get(buggyTracePath))) {
      System.out.println(
          "[DPOR Strategy Message] : Directory " + buggyTracePath + " does not exist ");
      System.exit(0);
    }
    initMcGraphs();
    initCurrentGraph();
   ...
   *
   * <p>This method initializes the {@link #currentGraph} with the {@link InitializationEvent}.
   */
  ...
  private void initGuidingGraph() {
    if (!RuntimeEnvironment.mcGraphs.isEmpty()) {
      ...
      System.out.println(
          "[DPOR Message] : The guiding execution graph is : G_" + guidingExecutionGraph.getId());
   ...
   *
   * <p>This method adds the event to the current graph. It sets the {@link #currentGraph} with the
   * given event.
   *
   ...
   *
   * <p>This method passes the event to the dpor model checker. It sets the {@link
   * DPOR#getAllGraphs()} ()} with an empty list to make sure that the {@link #dpor} model checker
   * does not have any previous graphs. Then, it calls the {@link DPOR#visit(ExecutionGraph, List)}
   * method to visit the current graph with the given event.
   *
   ...
   *
   * <p>This method updates the {@link #currentGraph}. It sets the {@link #currentGraph} with the
   * new graphs that are available in the {@link DPOR#getAllGraphs()}. If there is only one new
   * graph, it sets the {@link #currentGraph} with the new graph. If there are more than one new
   * graphs, it finds the extending graph. Otherwise, it creates a new graph and sets the {@link
   * #currentGraph} with the new graph.
   *
  ...
  protected void updateCurrentGraph(ThreadEvent threadEvent) {
    ...
    if (newGraphs.size() == 1) {
      ...
      //            System.out.println("[DPOR Strategy Debugging Message] : The events of the
      // visited graph are : ");
      //            currentGraph.getGraphEvents().forEach(System.out::println);
      //            System.out.println("[DPOR Strategy Debugging Message] : The COs of the visited
      // graph are : ");
      //            currentGraph.getCOs().forEach();
      //            System.out.println("[DPOR Strategy Debugging Message] : The STs of the visited
      // graph are : ");
      //            currentGraph.getSTs().forEach(System.out::println);
      //            System.out.println("[DPOR Strategy Debugging Message] : The JTs of the visited
      // graph are : ");
      //            currentGraph.getJTs().forEach(System.out::println);
      //            System.out.println("[DPOR Strategy Debugging Message] : The TCs of the visited
      // graph are : ");
      //            currentGraph.getTCs().forEach(System.out::println);
      //            System.out.println("[DPOR Strategy Debugging Message] : The events order of the
      // visited graph are : ");
      //            currentGraph.getEventsOrder().forEach(System.out::println);
    }
  }

  private void findExtendingGraphOverSymbolicEvent(
      List<ExecutionGraph> newGraphs, SymExecutionEvent symExecutionEvent) {
    for (ExecutionGraph graph : newGraphs) {
      ThreadEvent lastEvent =
          (ThreadEvent) graph.getGraphEvents().get(graph.getGraphEvents().size() - 1);
      ...
      if (lastSymEvent.getResult() == symExecutionEvent.getResult()) {
        ...
        System.out.println(
            "[DPOR Strategy Message] : The chosen graph is : " + currentGraph.getId());
        break;
      }
    }
  }
   ...
   *
   * <p>This method finds the extending graph of the current graph. It adds the new graphs to the
   * {@link #mcGraphs} list. Then, it iterates over the {@link ExecutionGraph#getSc()} of each new
   * graph and checks whether the left hand side of the pair is equal to the given thread event. If
   * it is not equal, it sets the {@link #currentGraph} with the new graph. Otherwise, it removes
   * the new graph from the {@link #mcGraphs} list.
   *
   ...
   *
   * <p>This method represents the required strategy for the next start event. It creates a {@link
   * StartEvent} for the corresponding starting a thread request of a thread. The created {@link
   * StartEvent} is added to the {@link #currentGraph} if the {@link #guidingActivate} is true.
   * Otherwise, it passes the event to the {@link #dpor} model checker.
   *
  ...
  public void nextStartEvent(Thread calleeThread, Thread callerThread) {
    ...
    if (guidingActivate) {
      ...
      System.out.println(
          "[DPOR Strategy Message] : The start event is passed to the model checker");
      passEventToDPOR(st);
  ...
  public void nextMainStartEvent(MainStartEvent mainStartEvent) {
    ...
    if (guidingActivate) {
      ...
      System.out.println(
          "[DPOR Strategy Message] : The main start event is passed to the model checker");
  ...
  public void nextConAssumeRequest(ConAssumeEvent conAssumeEvent) {
    ...
    if (guidingActivate) {
      ...
      System.out.println(
          "[DPOR Strategy Message] : The concrete assume event is passed to the model checker");
  ...
  public void nextAssumeBlockedRequest(AssumeBlockedEvent assumeBlockedEvent) {
    ...
    if (guidingActivate) {
      ...
      System.out.println(
          "[DPOR Strategy Message] : The assume blocked event is passed to the model checker");
   ...
   *
   * <p>This method represents the required strategy for the next join event. It creates a {@link
   * JoinEvent} for the corresponding joining a thread request of a thread and records it. The
   * created {@link JoinEvent} is added to the {@link #currentGraph} if the {@link #guidingActivate}
   * is true. Otherwise, it passes the event to the {@link #dpor} model checker.
   *
   ...
   *
   * <p>This method represents the required strategy for the next join request. It calls the {@link
   * #nextJoinEvent(Thread, Thread)} if the {@link #guidingActivate} is true. Otherwise, it puts the
   * join request and join response in the {@link RuntimeEnvironment#joinRequest} map and picks the
   * next random thread.
   *
   * @param joinReq is the thread that is going to join another thread.
   * @param joinRes is the thread that is going to be joined by another thread.
   ...
   *
   * <p>This method represents the required strategy for the next finish event. It creates a {@link
   * FinishEvent} for the corresponding finishing execution request of a thread and records it. The
   * created {@link FinishEvent} is added to the {@link #currentGraph} if the {@link
   * #guidingActivate} is true. Otherwise, it passes the event to the {@link #dpor} model checker.
   ...
   *
   * <p>This method represents the required strategy for the next finish request. It calls the
   * {@link #nextFinishEvent(Thread)} and picks the next thread based on the guiding execution graph
   * if the {@link #guidingActivate} is true. Otherwise, it picks the next random thread.
   *
   * @param thread is the thread that is going to be finished.
   * @return the next thread that is going to be executed.
   */
   ...
   *
   * <p>This method represents the required strategy for the next failure event. It creates a {@link
   * FailureEvent} for the corresponding failing execution request of a thread and records it. The
   * created {@link FailureEvent} is added to the {@link #currentGraph} if the {@link
   * #guidingActivate} is true. Otherwise, it passes the event to the {@link #dpor} model checker.
   *
   ...
   *
   * <p>This method represents the required strategy for the next deadlock event. It creates a
   * {@link DeadlockEvent} for the corresponding deadlock between the threads and records it. The
   * created {@link DeadlockEvent} is added to the {@link #currentGraph} if the {@link
   * #guidingActivate} is true. Otherwise, it passes the event to the
   *
   * @param thread is the thread that is going to be finished.
   */
  @Override
   ...
   *
   * <p>This method represents the required strategy for the next symbolic operation request. It
   * calls the {@link #handleGuidingSymbolicOperationRequest(Thread, SymbolicOperation)} if the
   * {@link #guidingActivate} is true. Otherwise, it calls the {@link
   ...
   *
   * <p>This method handles the new symbolic operation request. It finds the dependent formulas of
   ...
   * formula of the symbolic operation, and the negatable value. Finally, it adds the created event
   * to the {@link RuntimeEnvironment#eventsRecord} and passes the event to the dpor.
   *
   ...
   *
   * <p>This method picks the next thread that is going to be executed. It returns the next guided
   * thread if the {@link #guidingActivate} is true. Otherwise, it returns the next random thread.
   *
   * @return the next thread that is going to be executed.
   */
  @Override
   ...
   *
   * <p>This method saves the execution graphs to a file. It creates a new file with the given file
   * name and writes the execution graphs to the file.
   *
   ...
   *
   * <p>This method loads the execution graphs from a file. It reads the execution graphs from the
   * file and returns the loaded graphs.
   *
  ...
  public void saveBuggyExecutionTrace() {
    try {
      ...
      fileOut.close();
      System.out.println(
          "[DPOR Strategy Message] : Buggy execution trace is saved in "
              + buggyTracePath
              + buggyTraceFile);
    } catch (IOException i) {
      i.printStackTrace();
   ...
   *
   * <p>This method saves the execution state. It saves the {@link #mcGraphs} to the file and loads
   * the saved graphs to the {@link RuntimeEnvironment#mcGraphs} list. It also sets the {@link
   * RuntimeEnvironment#numOfGraphs} with the number of graphs that are available in the {@link
   * DPOR#getGraphCounter()}.
   */
  @Override
   ...
   *
   * <p>This method makes a new copy of the current graph's COs. It iterates over the {@link
   * ExecutionGraph#getCOs()} of the {@link #guidingExecutionGraph} and finds the new COs based on
   * the current graph. It returns the new COs.
   *
   ...
   *
   * <p>This method finds the write event based on the current graph. It returns the write event
   * that is available in the {@link #currentGraph}.
   *
  ...
  protected WriteEvent findWriteEvent(WriteEvent tempWrite) {
    return currentGraph.getGraphEvents().stream()
        ...
        .filter(
            writeEvent ->
                writeEvent.getTid() == tempWrite.getTid()
   ...
   *
   * <p>This method makes a new copy of the current graph's STs. It iterates over the {@link
   * ExecutionGraph#getSTs()} of the {@link #guidingExecutionGraph} and finds the new STs based on
   * the current graph. It returns the new STs.
   *
   ...
   *
   * <p>This method makes a new copy of the current graph's MCs. It iterates over the {@link
   * ExecutionGraph#getMCs()} of the {@link #guidingExecutionGraph} and finds the new MCs based on
   * the current graph. It returns the new MCs.
   *
   ...
   *
   * <p>This method makes a new copy of the current graph's TCs. It iterates over the {@link
   * ExecutionGraph#getTCs()} of the {@link #guidingExecutionGraph} and finds the new TCs based on
   * the current graph. It returns the new TCs.
   *
   ...
   *
   * <p>This method makes a new copy of the current graph's PCs. It iterates over the {@link
   * ExecutionGraph#getPCs()} of the {@link #guidingExecutionGraph} and finds the new PCs based on
   * the current graph. It returns the new PCs.
   *
  ...
  protected Set<Pair<Event, Event>> findNewRecvfrom() {
    ...
    for (Pair<Event, Event> recvfrom : guidingExecutionGraph.getRecvFrom()) {
      ThreadEvent firstThreadEvent =
          findThreadEventInCurrentGraph((ThreadEvent) recvfrom.component1());
      ThreadEvent secondThreadEvent =
          findThreadEventInCurrentGraph((ThreadEvent) recvfrom.component2());
      if (firstThreadEvent != null && secondThreadEvent != null) {
   ...
   *
   * <p>This method finds the thread event based on the current graph. It returns the thread event
   * that is available in the {@link #currentGraph}.
   *
  ...
  protected ThreadEvent findThreadEventInCurrentGraph(ThreadEvent tempThreadEvent) {
    return currentGraph.getGraphEvents().stream()
        ...
        .filter(
            threadEvent ->
                threadEvent.getTid() == tempThreadEvent.getTid()
   ...
   *
   * <p>This method makes a new copy of the current graph's JTs. It iterates over the {@link
   * ExecutionGraph#getJTs()} of the {@link #guidingExecutionGraph} and finds the new JTs based on
   * the current graph. It returns the new JTs.
   *
   ...
   *
   * <p>This method finds the guiding thread from the start event. It returns the thread id of the
   * start event that is available in the {@link #guidingExecutionGraph}.
   *
