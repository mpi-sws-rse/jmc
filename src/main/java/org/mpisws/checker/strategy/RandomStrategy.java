...
public class RandomStrategy implements SearchStrategy {
     ...
     *
     * <p>This method handles the next symbolic operation request of a given thread. It checks if
     * the symbolic operation is dependent on other formulas. If the symbolic operation is
     * dependent, it creates a dependency operation and solves the dependent formulas. If the
     * symbolic operation is free from dependencies, it solves the formula. The method updates the
     * path symbolic operations and creates a {@link SymExecutionEvent} for the symbolic operation.
     *
    ...
    /**
     * Creates an {@link ExitMonitorEvent} for the corresponding exiting a monitor request of a
     * thread
     *
     ...
     * ParkEvent} for the corresponding parking request of a thread and records it. The method also
     * checks if the thread has a parking permit. If the thread has a parking permit, it creates an
     * {@link UnparkEvent} for the corresponding unparking request of the thread and records it. If
     * the thread does not have a parking permit, it parks the thread.
     *
     ...
     * UnparkingEvent} for the corresponding unparking request of a thread and records it. The
     * method also checks if the unparkee thread is parked. If the unparkee thread is parked, it
     * unparks the thread and creates an {@link UnparkEvent} for the corresponding unparking request
     * of the thread and records it. If the unparkee thread is not parked, it grants the thread a
     * parking permit.
     *
