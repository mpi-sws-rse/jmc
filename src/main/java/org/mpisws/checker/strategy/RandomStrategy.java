...
/**
 * The RandomStrategy class implements the {@link SearchStrategy} interface and is responsible for
 * managing the execution order of events in a multithreaded program using a random strategy. It
...
public class RandomStrategy implements SearchStrategy {
   ...
   * RuntimeEnvironment#buggyTracePath}, and {@link #random} with a new random number generator with
   * the seed value from {@link RuntimeEnvironment#seed}, and {@link #solver} with the value from
  ...
  public RandomStrategy() {
    ...
    if (!Files.exists(Paths.get(buggyTracePath))) {
      System.out.println(
          "[Random Strategy Message] : Directory " + buggyTracePath + " does not exist ");
      System.exit(0);
    }
   ...
   *
   * <p>This method creates a {@link StartEvent} for the corresponding starting a thread request of
   * a thread. The created {@link StartEvent} is added to the {@link
  ...
  public void nextSymAssumeRequest(Thread thread, SymbolicOperation symbolicOperation) {
    ...
    System.out.println(
        "[Random Strategy Message] : The result of the symbolic assume operation is "
   ...
   *
   * <p>This method handles the next symbolic operation request of a given thread. It checks if the
   * symbolic operation is dependent on other formulas. If the symbolic operation is dependent, it
   * creates a dependency operation and solves the dependent formulas. If the symbolic operation is
   * free from dependencies, it solves the formula. The method updates the path symbolic operations
   * and creates a {@link SymExecutionEvent} for the symbolic operation.
   *
  ...
  public void nextSymbolicOperationRequest(Thread thread, SymbolicOperation symbolicOperation) {
    ...
    System.out.println(
        "[Random Strategy Message] : The result of the symbolic arithmetic operation is "
            + RuntimeEnvironment.solverResult);
  ...
  /**
   * Creates an {@link EnterMonitorEvent} for the corresponding entering a monitor request of a
   * thread
   *
   ...
   *
   * <p>This method creates a {@link JoinEvent} for the corresponding joining a thread request of a
   * thread. The created {@link JoinEvent} is added to the {@link RuntimeEnvironment#eventsRecord}.
   *
   ...
   * for the corresponding parking request of a thread and records it. The method also checks if the
   * thread has a parking permit. If the thread has a parking permit, it creates an {@link
   * UnparkEvent} for the corresponding unparking request of the thread and records it. If the
   * thread does not have a parking permit, it parks the thread.
   *
   ...
   *
   * <p>This method handles the next unpark request of a given thread. It creates an {@link
   * UnparkingEvent} for the corresponding unparking request of a thread and records it. The method
   * also checks if the unparkee thread is parked. If the unparkee thread is parked, it unparks the
   * thread and creates an {@link UnparkEvent} for the corresponding unparking request of the thread
   * and records it. If the unparkee thread is not parked, it grants the thread a parking permit.
   *
   ...
   *
   * <p>This method handles the next join request of a given thread. It records the join request and
   * join response threads in the {@link RuntimeEnvironment#joinRequest} map. The method also
  ...
  /**
   * Records the created {@link ReadEvent} for the corresponding reading a variable request of a
   * thread
   *
  ...
  /**
   * Records the created {@link WriteEvent} for the corresponding writing a variable request of a
   * thread
   *
   * <p>This method records the created {@link WriteEvent} for the corresponding writing a variable
   * request of a thread. The {@link WriteEvent} is added to the {@link
   * RuntimeEnvironment#eventsRecord}.
   *
  ...
  private boolean handleFreeBlockingRecvReq(
  ...
  private boolean handleConditionalBlockingRecvReq(
      ReceiveEvent receiveEvent, JMCThread jmcThread, BlockingRecvReq blockingRecvReq) {
   ...
   *
   * <p>This method creates a {@link FinishEvent} for the corresponding finishing execution request
   * of a thread. The created {@link FinishEvent} is added to the {@link
   ...
   *
   * <p>This method calls the {@link #nextFinishEvent(Thread)} method to handle the next finish
   * request of a given thread. The method also selects the next random thread to run.
   *
   * @param thread is the thread that is going to be finished.
   * @return the next random thread to run.
   */
   ...
   *
   * <p>This method creates a {@link FailureEvent} for the corresponding failure event of a thread.
   * The created {@link FailureEvent} is added to the {@link RuntimeEnvironment#eventsRecord}.
   *
   ...
   *
   * <p>This method saves the buggy execution trace in the buggy trace file.
   */
  @Override
  public void saveBuggyExecutionTrace() {
    try {
      ...
      fileOut.close();
      System.out.println(
          "[Random Strategy Message] : Buggy execution trace is saved in "
              + buggyTracePath
              + buggyTraceFile);
    } catch (IOException i) {
      i.printStackTrace();
