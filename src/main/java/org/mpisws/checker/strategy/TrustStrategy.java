...
public class TrustStrategy extends DPORStrategy {
     ...
     *
     * <p>This method handles the next park request of a given thread. It creates a {@link
     * ParkEvent} for the corresponding parking request of a thread and records it. If the
     * scheduling is in the guided mode, it adds the event to the current graph. Also, it checks
     * whether the thread has the parking permit or not. If the thread has the parking permit, it
     * sets the parking permit to false. Otherwise, it parks the thread. If the scheduling is normal
     * mode, it checks whether the thread has the parking permit or not. If the thread has the
     * parking permit, it sets the parking permit to false. Then, it creates a {@link UnparkEvent}
     * for the corresponding unparking request of the thread and records it. Finally, it passes the
     * events to the trust model checker and updates the current graph. If the thread does not have
     * the parking permit, it parks the thread. Then, it passes the park event to the trust model
     * checker.
     *
     ...
     * scheduling is in the guided mode, it adds the event to the current graph. Also, it checks
     * whether the unparkee thread is parked or not. If the unparkee thread is parked, it unparks
     * the unparkee thread. Otherwise, it sets the parking permit of the unparkee thread to true. If
     * the scheduling is normal mode, it checks whether the unparkee thread is parked or not. If the
     * unparkee thread is parked, it unparks the unparkee thread, creates a {@link UnparkEvent} for
     * the corresponding unparking request of the unparkee thread and records it. Then, it passes
     * the events to the trust model checker and updates the current graph. If the unparkee thread
     * is not parked, it sets the parking permit of the unparkee thread to true, creates a {@link
            ...
            readsFrom =
                    currentGraph.getGraphEvents().stream()
                            ...
                            .filter(
                                    writeEvent ->
                                            writeEvent.getTid()
                                                            == Objects.requireNonNull(tempWrite)
                                                                    .getTid()
                                                    && writeEvent.getSerial()
                                                            == tempWrite.getSerial())
                            .findFirst()
                            .orElse(null);
        }
            ...
            readsFrom =
                    currentGraph.getGraphEvents().stream()
                            ...
                            .filter(
                                    writeExEvent ->
                                            writeExEvent.getTid()
                                                            == Objects.requireNonNull(tempWriteEx)
                                                                    .getTid()
                                                    && writeExEvent.getSerial()
                                                            == tempWriteEx.getSerial())
                            .findFirst()
                            .orElse(null);
        }
        return readsFrom;
                ...
                .noneMatch(
                        pair ->
                                pair.component1().getType() == threadEvent.getType()
                                        && ((ThreadEvent) pair.component1()).getTid()
                                                == threadEvent.getTid()
                                        && ((ThreadEvent) pair.component1()).getSerial()
                                                == threadEvent.getSerial());
    }
    ...
    private void updateCurrentGraph(ReadExEvent readExEvent, WriteExEvent writeExEvent) {
        ...
        if (newGraphs.size() == 1) {
            ...
            System.out.println(
                    "[Trust Strategy Message] : There is no new graph from model checker");
            System.out.println(
    ...
    private void findExtendingGraph(
        ...
        System.out.println(
                "[Trust Strategy Message] : The chosen graph is : " + currentGraph.getId());
    }
                ...
                .noneMatch(
                        pair ->
                                pair.component1().getType() == readExEvent.getType()
                                        && ((ReadExEvent) pair.component1()).getTid()
                                                == readExEvent.getTid()
                                        && ((ReadExEvent) pair.component1()).getSerial()
                                                == readExEvent.getSerial()
                                        && !(pair.component2().getType() == writeExEvent.getType()
                                                && ((WriteExEvent) pair.component2()).getTid()
                                                        == writeExEvent.getTid()
    ...
    public Thread nextCasRequest(
        ...
        if (RuntimeEnvironment.monitorList.containsKey(monitor)) {
            ...
            if (monitorsDeadlockDetection()) {
                System.out.println(
                        "[Trust Strategy Message] : There is a deadlock between the threads in"
                            + " using the monitors");
     ...
     *
     * <p>This method represents the required strategy for the next enter monitor request. It
     * creates a {@link MonitorRequestEvent} for the corresponding entering a monitor request of a
     * thread and records it. The created {@link MonitorRequestEvent} is added to the {@link
     ...
     *
     * <p>This method represents the required strategy for the next suspend event. It creates a
     * {@link SuspendEvent} for the corresponding suspending execution request of a thread and
     * records it. The created {@link SuspendEvent} is added to the {@link #currentGraph}.
     *
     ...
     * empty or not. If it is empty, it calls the {@link #handleEmptyGuidingEvents()} method and
     * picks the next random thread. Otherwise, it picks the next guided thread based on the {@link
     ...
     * guided thread. If the {@link #guidingEvent} is an instance of {@link StartEvent}, it finds
     * the guiding thread from the {@link #guidingExecutionGraph}. Otherwise, it sets the {@link
     ...
     * instance of {@link SuspendEvent}, it calls the {@link
     * #guidedSuspendEventHelper(SuspendEvent)} method and picks the next guided thread. If the
     * {@link #guidingEvent} is an instance of {@link UnparkEvent}, it calls the {@link
     * #guidedUnparkEventHelper(UnparkEvent)} method and returns the unparked thread. Otherwise, it
     * returns the next guided thread.
     *
    ...
    public Thread pickNextGuidedThread() {
        ...
        if (guidingEvent instanceof StartEvent) {
            ...
            System.out.println(
                    "[Trust Strategy Debugging] The STs of graph is:"
                            + guidingExecutionGraph.getSTs());
            System.out.println(
     ...
     *
     * <p>This method handles the empty guiding events. It prints a message that the guiding events
     * is empty and finds the new COs, STs, JTs, MCs, and TCs based on the current graph. Then, it
     * sets the {@link #guidingActivate} to false.
     */
    @Override
     ...
     * #findForbiddenThreads(Object)} method. Finally, it unsuspends the candidate threads if they
     * are not in the forbidden threads list.
     *
     ...
     * #guidingExecutionGraph}, it finds the order of the events that if it picks the first event,
     * the second event does not have any left hand side in the {@link ExecutionGraph#getSc()} of
     * the {@link #guidingExecutionGraph} except the first event. <br>
