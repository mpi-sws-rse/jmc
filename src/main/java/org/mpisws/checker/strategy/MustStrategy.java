...
public class MustStrategy extends DPORStrategy {
   ...
   *
   * <p>This method initializes the {@link #dpor} object. It sets the {@link Must#getGraphCounter()}
   * with the number of graphs that are available in the {@link RuntimeEnvironment#mcGraphs}.
   */
  ...
  private void executeReceiveEvent(ReceiveEvent receiveEvent) {
    ReceiveEvent tempReceive =
        (ReceiveEvent) currentGraph.getGraphEvents().get(currentGraph.getGraphEvents().size() - 1);
   ...
   *
   * <p>This method adds the receive-from edge to the {@link #currentGraph}. It sets the
   * receive-from edge of the receive event to the {@link #currentGraph}.
   *
  ...
  private void addRfEdgeToCurrentGraph(ReceiveEvent receiveEvent) {
    Optional<ReceivesFrom> receiveFrom =
        Optional.ofNullable(findRfEdge((ReceiveEvent) guidingEvent));
    ...
    if (receiveEvent.getRf() instanceof InitializationEvent) {
      ...
      receivesFrom =
          currentGraph.getGraphEvents().stream()
              ...
              .filter(
                  sendEvent ->
                      sendEvent.getTid() == Objects.requireNonNull(tempSend).getTid()
  ...
  private ReceiveEvent findReceiveEventFromBlockingRecvReq(ReceiveEvent guidedReceiveEvent) {
    ...
    for (Event event : RuntimeEnvironment.eventsRecord) {
      if (event instanceof BlockingRecvReq blockingRecvReq) {
        System.out.println(
            "[Debugging Message in Must] : Blocking Receive Request - " + blockingRecvReq);
   ...
   * empty and finds the new RecvFrom, STs, JTs, and TCs based on the current graph. Then, it sets
  ...
  public void findGuidingEvents() {
    ...
    while (!remainingEvents.isEmpty()) {
      remainingEvents.removeIf(
          event -> {
            boolean isEventFree =
                guidingExecutionGraph.getPorf().stream()
                    .noneMatch(
                        pair ->
