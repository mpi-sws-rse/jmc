...
public class ReplayStrategy implements SearchStrategy {
   ...
   *
   * <p>It initializes the {@link #buggyTracePath} and {@link #buggyTraceFile} properties and loads
   * the guiding trace.
   */
   ...
   * the unparkee thread is parked or not. If the unparkee thread is parked, it unparks the thread.
   * Otherwise, it sets the parking permit of the unparkee thread to true.
   *
  ...
  private void handleGuidedRecvEvent(ReceiveEvent receiveEvent, ReceiveEvent guidingRecvEvent) {
    ...
    // Assign receiveFrom with the send event in the RuntimeEnvironment.eventsRecord that has the
    ...
    if (receiveFrom == null) {
      System.out.println(
          "[Replay Strategy Message] : The receive event does not have a corresponding send event");
      System.exit(0);
    }
   ...
   * event. Otherwise, it sets the guiding thread to the thread of the guiding event. If the guiding
   * event is an enter monitor event, it removes the monitor request from the monitor request list
   * and calls the {@link #nextEnterMonitorEvent(Thread, Object)} method to add the enter monitor
   * event to the execution trace. If the guiding event is an unpark event, it calls the {@link
   * #guidedUnparkEventHelper(UnparkEvent)} method to handle the unpark event. Otherwise, it prints
   * a message and returns the thread of the guiding event.
   *
  ...
  public Thread pickNextThread() {
    if (guidingTrace.isEmpty()) {
      ...
      System.out.println(
          "[Replay Strategy Message] : Thus, there is something wrong with the Replay Strategy");
      printExecutionTrace();
      System.exit(0);
    }
  ...
  private void handleNextGuidedBlockedRecvEvent(BlockedRecvEvent guidedBlockedRecvEvent) {
    ...
    System.out.println(
        "[Replay Strategy Message] : The recv event in the record is: " + receiveEventInRecord);
    JMCThread jmcThread =
   ...
   * #nextEnterMonitorEvent(Thread, Object)} method to add the enter monitor event to the execution
   * trace.
   *
