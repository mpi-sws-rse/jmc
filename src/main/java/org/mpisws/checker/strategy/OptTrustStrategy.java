...
public class OptTrustStrategy extends OptDPORStrategy {
    ...
    protected void handleGraphOp(GraphOp graphOp) {
        switch (graphOp.getType()) {
            case FR_L_W:
                makeDPORFree();
                dpor.processFR_L_W(graphOp.getG(), (WriteEvent) graphOp.getFirstEvent(), graphOp.getToBeAddedEvents());
                break;
            case FR_W_W:
                makeDPORFree();
                dpor.processFR_W_W(graphOp.getG(), (WriteEvent) graphOp.getFirstEvent(), (WriteEvent) graphOp.getSecondEvent(), graphOp.getToBeAddedEvents());
                break;
            case FR_R_W:
                makeDPORFree();
                dpor.processFR_R_W(graphOp.getG(), (ReadEvent) graphOp.getFirstEvent(), (WriteEvent) graphOp.getSecondEvent());
                break;
            case FR_RX_W:
                makeDPORFree();
                dpor.processFR_RX_W(graphOp.getG(), (ReadExEvent) graphOp.getFirstEvent(), (WriteEvent) graphOp.getSecondEvent(), graphOp.getToBeAddedEvents());
                break;
            case FR_NEG_SYM:
                makeDPORFree();
