...
public class SimpleSolver extends SymbolicSolver {
   ...
   * #disSolveSymbolicFormula(SymbolicOperation)} methods to solve the symbolic operation. Then, it
   * calls the {@link #pickSatOrUnsat(boolean, boolean)} method to pick the result of the symbolic
   * operation. Finally, it sets the {@link RuntimeEnvironment#solverResult} with the result of the
   * symbolic operation.
   *
  ...
  private void handleFreeFormulas(SymbolicOperation symbolicOperation) {
    System.out.println(
        "[Simple Symbolic Solver Message] : The symbolic arithmetic operation is free from"
            + " dependencies");
   ...
   *
   * <p>This method finds the dependent formulas of the given symbolic operation. It iterates over
   * the path symbolic operations and checks whether the given symbolic operation is dependent on
   * another symbolic operation. If it is dependent, it adds the dependent symbolic operation to the
   * list of dependent formulas. Otherwise, it returns null.
   *
  ...
  private List<SymbolicOperation> findDependentFormulas(SymbolicOperation symbolicOperation) {
    ...
    for (SymbolicOperation symOp : symbolicOperations) {
      if (symOp.isFormulaDependent(symbolicOperation)) {
        System.out.println(
            "[Simple Symbolic Solver Message] : The symbolic arithmetic operation is dependent on "
                + "another symbolic arithmetic operation");
        System.out.println(
            "[Simple Symbolic Solver Message] : The dependent symbolic arithmetic operations are : "
   ...
   * symbolic operation is dependent on another symbolic operation. If it is dependent, it adds the
   * dependent symbolic operation to the list of dependent formulas. Otherwise, it returns null.
   *
   ...
   *
   * <p>This method handles the dependent formulas. First, it creates a dependency operation from
   ...
   * #disSolveDependentSymbolicFormulas(SymbolicOperation, SymbolicOperation)} methods to solve the
   * symbolic operation. Then, it calls the {@link #pickSatOrUnsat(boolean, boolean)} method to pick
   * the result of the symbolic operation. Finally, it sets the {@link
  ...
  private void handleDependentFormulas(
    ...
    System.out.println(
        "[Simple Symbolic Solver Message] : The symbolic arithmetic operation has dependencies");
    SymbolicOperation dependency = makeDependencyOperation(dependentOperations);
  ...
  protected boolean solver(BooleanFormula formula) {
    try (ProverEnvironment prover =
      ...
      System.out.println(
          "[Simple Symbolic Solver Message] : Solving the formula: " + formula.toString());
  ...
  private SymbolicOperation negateSymbolicOperation(SymbolicOperation symbolicOperation) {
    ...
    System.out.println(
        "[Simple Symbolic Solver Message] : The negated formula is saved "
            + symbolicOperation.getFormula());
