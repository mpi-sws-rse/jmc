...
/**
 * The SymbolicSolver class is a core component designed for solving symbolic formulas within the
 * program under the test. It encapsulates the functionality for manipulating and solving symbolic
 * representations of boolean and integer variables. This class leverages the JavaSMT API to
 * interface with various SMT (Satisfiability Modulo Theories) solvers, providing a unified API for
 * theorem proving tasks.
 *
 * <p>The class supports operations such as solving symbolic formulas, negating formulas, and
 * creating dependency formulas from a list of symbolic operations. It allows for the dynamic
 * selection of SMT solvers based on the provided solver type, enabling flexibility in choosing the
 * underlying solver based on the specific requirements of the verification task.
 *
 * <p>SymbolicSolver initializes with a configuration, a logging manager, and a shutdown manager to
 * handle the lifecycle of the solver context. It provides methods to access and manipulate symbolic
 * variables (both boolean and integer), facilitating the creation of complex symbolic expressions
 * that can be used by programmer.
 */
public abstract class SymbolicSolver {
    ...
    /**
     * @property {@link #symIntVariableMap} used to store the mapping of symbolic integer variables
     *     to their names.
     */
    public Map<String, SymIntVariable> symIntVariableMap = new HashMap<>();

    /**
     * @property {@link #symBoolVariableMap} used to store the mapping of symbolic boolean variables
     *     to their names.
     */
    public Map<String, SymBoolVariable> symBoolVariableMap = new HashMap<>();

    /**
     * @property {@link #context} used to store the solver context for interacting with the SMT
     *     solver.
     */
    protected SolverContext context;

    /**
     * @property {@link #fmgr} used to access the formula manager for creating and manipulating
     *     formulas.
     */
    ...
    /**
     * @property {@link #model} used to store the model generated by the solver for a satisfiable
     *     formula.
     */
    ...
    /**
     * Constructs a new instance of the SymbolicSolver class with the default SMT solver type
     * (SMTInterpol).
     */
    ...
    /**
     * Finds the corresponding SolverContextFactory.Solvers enum value for the given SMTSolverTypes
     * enum value.
     *
    ...
    /**
     * Solves the symbolic operation with conjunction of the given dependency operation and returns
     * the result.
     *
     ...
     */
    public boolean solveDependentSymbolicFormulas(
            SymbolicOperation operation, SymbolicOperation dependencyOperation) {
        BooleanFormula contextFormula =
                bmgr.and(operation.getFormula(), dependencyOperation.getFormula());
        return solver(contextFormula);
    }

    /**
     * Dissolves the symbolic operation with conjunction of the given dependency operation and
     * returns the result.
     *
     ...
     */
    public boolean disSolveDependentSymbolicFormulas(
            SymbolicOperation operation, SymbolicOperation dependencyOperation) {
        BooleanFormula contextFormula =
                bmgr.and(negateFormula(operation.getFormula()), dependencyOperation.getFormula());
        return solver(contextFormula);
    }
    ...
    public SymbolicOperation makeDependencyOperation(List<SymbolicOperation> operations) {
        ...
        System.out.println(
                "[Debugging Message] : Dependency Operation: "
                        + dependencyOperation.getFormula().toString());
    ...
    protected boolean pickSatOrUnsat(boolean sat, boolean unSat) {
        if (sat && unSat) {
            System.out.println(
                    "[Symbolic Solver Message] : Both SAT and UNSAT are possible for the symbolic "
                            + "arithmetic operation");
            return new Random().nextBoolean();
        } else if (sat) {
            System.out.println(
                    "[Symbolic Solver Message] : Only SAT is possible for the symbolic arithmetic "
                            + "operation");
            return true;
        } else if (unSat) {
            System.out.println(
                    "[Symbolic Solver Message] : Only UNSAT is possible for the symbolic arithmetic"
                        + " operation");
            return false;
        } else {
            System.out.println(
                    "[Symbolic Solver Message] : No solution is found for the symbolic arithmetic "
                            + "operation");
            System.exit(0);
            return false;
        }
     ...
     *
     * <p>This method creates a new prover environment, adds the boolean formula as a constraint,
     * and checks if the formula is satisfiable. If the formula is satisfiable, the model generated
     * by the solver is stored in the model property. The method returns true if the formula is
     ...
     *
     * <p>This method checks if the symbolic integer variable with the given name already exists in
     * the symIntVariableMap. If the variable exists, it returns the existing variable. Otherwise,
     * it creates a new symbolic integer variable with the given name, stores it in the
     * symIntVariableMap, and returns the new variable.
     *
    ...
    public SymIntVariable getSymIntVariable(String name) {
        if (symIntVariableMap.containsKey(name)) {
            System.out.println(
                    "[Symbolic Solver Message] Returning existing SymIntVariable: " + name);
    ...
    public int getSymIntVarValue(String name) {
        if (symIntVariableMap.containsKey(name)) {
            ...
            throw new IllegalArgumentException(
                    "[Symbolic Solver Message] Symbolic integer variable not found: " + name);
        }
    }
     ...
     *
     * <p>This method checks if the symbolic boolean variable with the given name already exists in
     * the symBoolVariableMap. If the variable exists, it returns the existing variable. Otherwise,
     * it creates a new symbolic boolean variable with the given name, stores it in the
     * symBoolVariableMap, and returns the new variable.
     *
    ...
    public boolean getSymBoolVarValue(String name) {
        if (symBoolVariableMap.containsKey(name)) {
            ...
            throw new IllegalArgumentException(
                    "[Symbolic Solver Message] Symbolic boolean variable not found: " + name);
        }
    }
    ...
    public abstract void computeRandomlyNewSymbolicOperationRequest(
