...
public class SymbolicSolver {
    ...
    private boolean solver(BooleanFormula formula) {
        try (ProverEnvironment prover = context.newProverEnvironment(SolverContext.ProverOptions.GENERATE_MODELS)) {
            System.out.println("[Symbolic Solver Message] : Solving the formula: " + formula.toString());
            prover.addConstraint(formula);
            boolean isUnsat = prover.isUnsat();
            if (!isUnsat) {
                model = prover.getModel();
                evaluate();
                System.out.println("[Symbolic Solver Message] : The formula is satisfiable");
                prover.close();
                return true;
            } else {
                evaluate();
                System.out.println("[Symbolic Solver Message] : The formula is unsatisfiable");
                prover.close();
                return false;
            }
