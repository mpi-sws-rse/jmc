        ...
        if (model != null) {
            model.iterator()
                    .forEachRemaining(
                            entry -> {
                                LOGGER.debug(
                                        "Model Entry: {} : {}", entry.getKey(), entry.getValue());
                                // The key is a string like className@address. extract the class
                                // Name
                                ...
                                if (symbolicType.equals("SymbolicBoolean")) {
                                    ...
                                    if (entry.getValue() instanceof BigInteger) {
                                        symIntVariableMap
                                                .get(entry.getKey().toString())
                                                .setValue(
                                                        ((BigInteger) entry.getValue()).intValue());
                                    } else {
                                        symIntVariableMap
                                                .get(entry.getKey().toString())
                                    ...
                                    throw new RuntimeException(
                                            "[Incremental Solver Message] Unknown Symbolic Type");
                                }
                            });
        }
    }
        ...
        if (prover.size() > 0) {
            try {
                ...
                if (!isUnsat) {
                    ...
                    throw new RuntimeException(
                            "[Incremental Solver Message] The formula is unsatisfiable");
                }
