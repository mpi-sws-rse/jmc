...
public class SchedulerThread extends Thread {
  ...
  /**
   * The @property {@link SearchStrategy} is used to select the next thread to be executed based on
   * the selected strategy.
   */
  private SearchStrategy searchStrategy;

  /**
   * The following constructor initializes the {@link SearchStrategy} based on the selected strategy
   * type.
   */
   ...
   *
   * <p>The following method is used to initialize the {@link SearchStrategy} based on the selected
   * strategy type. If the selected strategy type is {@link StrategyType#RANDOM}, the method
   * initializes the {@link RandomStrategy}. Otherwise, the method throws an {@link
   ...
   *
   * <p>This method is used to execute the scheduler thread. It waits until the other thread (which
   * is the main thread) waits, prints a message indicating that the flow of the program is
   * controlled by the SchedulerThread, and then handles the events until the execution is finished.
   */
  @Override
  public void run() {
    waitForMainThread();
   ...
   *
   * <p>This method is used to check whether the wait request is due to an assert violation or not.
   *
   ...
   *
   * <p>This method is used to handle the assert fail. It sets the execution to be finished, prints
   * a message indicating that the SchedulerThread requested to finish, and then prints the executed
   * events.
   */
  ...
  private void printResourceUsage() {
    ...
    LOGGER.debug(
        "[*** Number of execution iteration : {} ***]", RuntimeEnvironment.numOfExecutions);
    LOGGER.debug(
        "[*** Number of threads created : {} ***]", RuntimeEnvironment.createdThreadList.size());
    LOGGER.debug(
        "[*** Memory used : {} MB ***]", RuntimeEnvironment.currentMemoryUsageInMegaBytes());
    ...
    LOGGER.debug(
        "[*** Time taken to execute the program : {} min: {} sec", timeInMinutes, timeInSeconds);
   ...
   *
   * <p>This method is used to wait until the state of the only other running thread changes to the
   * WAIT state.
   */
   ...
   *
   * <p>This method is used to notify the specified thread to continue its execution. If the thread
   * is null, it indicates that the execution is finished. In this case, the method sets the {@link
   * RuntimeEnvironment#executionFinished} flag to true. Otherwise, The method retrieves the
   * thread's ID, prints a message indicating that the thread is permitted to run, and then notifies
   * the thread.
   *
   ...
   * thread is permitted to run for loading in the runtime environment, and then starts the thread.
   *
   ...
   *
   * <p>This method is used to handle the enter monitor request of a thread. It adds the thread and
   * the monitor into the monitorRequest and calls the next enter monitor request method of the
   * search strategy. If the next thread to run is null, it calls the next deadlock event method of
   * the search strategy. Otherwise, it notifies the thread to run.
   */
   ...
   *
   * <p>This method is used to handle the exit monitor request of a thread. It retrieves the thread
   * and the monitor that requested to exit, handles the exit monitor event by calling the next exit
   * monitor event method of the search strategy, sets the thread exit monitor request and the
   * object exit monitor request to null, sets the thread wait request to null, and finally calls
   * the wait event handler, which selects the next thread to run.
   */
  ...
  public void joinRequestHandler() {
    ...
    if (joinRequestThread.isPresent() && joinResponseThread.isPresent()) {
      Optional<Thread> thread =
          Optional.ofNullable(
              searchStrategy.nextJoinRequest(joinRequestThread.get(), joinResponseThread.get()));
   ...
   * that requested by the thread, handles the write request, sets the write event request to null,
   * sets the thread wait request to null, and then notifies the thread.
   */
   ...
   * thread and the unparkee thread, sets the unparker thread and the unparkee thread to null, sets
   * the thread wait request to null, and then calls the next unpark request method of the search
   * strategy. Finally, it calls the wait event handler to select the next thread to run.
   */
   ...
   *
   * <p>This method is used to handle the symbolic arithmetic request of a thread. It retrieves the
   * thread that requested the symbolic arithmetic operation, sets the thread wait request to null,
   * and then calls the next symbolic operation request method of the search strategy. Finally, it
   * notifies the thread to run.
   */
