...
public class RuntimeEnvironment {
  ...
  /**
   * @property {@link #threadExitMonitorReq} is used to store the thread that requested to exit the
   *     monitor
   */
   ...
   *     assert statement is executed, and it failed. Thus, the {@link SchedulerThread} object will
  ...
  /**
   * * @property {@link #threadBlockingRecvList} is used to store the last blocking receive request
   * for each thread
   */
   ...
   *     monitor is available, the {@link SchedulerThread} selects the thread to run and removes the
   *     thread from {@link #monitorRequest}. Otherwise, the {@link SchedulerThread} selects another
   *     thread to run.
   */
  public static Map<Thread, Object> monitorRequest = new HashMap<>();

  /**
   * @property {@link #joinRequest} is used to store the threads that are waiting to join to another
   *     thread.
   *     <p>The {@link SchedulerThread} uses this map in scheduling phase in order to select the
   *     next thread to run which will not cause a deadlock. When a thread is selected to run, and
   ...
   *     selected thread wants to join is still alive or not. If the thread is finished, the {@link
   *     SchedulerThread} selects the thread to run and removes it from {@link #joinRequest}.
   *     Otherwise, the {@link SchedulerThread} selects another thread to run.
   */
  public static Map<Thread, Thread> joinRequest = new HashMap<>();

  /**
   * @property {@link #eventsRecord} is used to store the events that are executed by the threads in
   *     the program under test using the {@link org.mpisws.checker.strategy.RandomStrategy}.
   */
  ...
  /**
   * @property {@link #threadIdMap} is used to store the mapping between each thread id generated by
   *     JVM and the thread id generated by the {@link RuntimeEnvironment}.
   */
  public static Map<Long, Long> threadIdMap = new HashMap<>();

  /**
   * @property {@link #threadObjectMap} is used to store the mapping between the thread id generated
   *     by the {@link RuntimeEnvironment} and the thread object.
   */
   ...
   *     #RuntimeEnvironment} and {@link SchedulerThread}. The supported strategy types are: {@link
   *     StrategyType#RANDOM} and {@link StrategyType#TRUST}.
   */
  ...
  /**
   * @property {@link #allExecutionsFinished} is used to indicate that all execution iterations are
   *     finished.
   */
  ...
  /**
   * @property {@link #parkedThreadList} is used to store the threads that are parked in the program
   *     under test.
   */
   ...
   *     program under test, based on the monitor that the suspendee thread is waiting to enter and
   *     the monitor that the suspender thread has already entered. (Currently, this feature is only
   *     used by trust strategy to sustain the consistency of the scheduling with respect to the
   *     suspend Events)
   */
   ...
   *     the program under test. The key is the thread id and the value is the parking permit. When
   *     a thread is going to be parked or unparked, this field is used to check whether the thread
   *     has the parking permit or not.
   */
  ...
  /**
   * @property {@link #staticMethodMonitorList} is used to store the static method monitors that are
   ...
   *     lock. The value 0 indicates that the lock is available, and the value 1 indicates that the
  ...
  /**
   * Loads the {@link CheckerConfiguration} object from the config.obj file and assigns it to {@link
   * #config}.
   *
  ...
  public static AssignedTaskEvent createAssignedTaskEvent(Thread thread, Runnable task) {
    ...
    return new AssignedTaskEvent(
        EventType.ASSIGNED_TASK, threadIdMap.get(thread.getId()).intValue(), serialNumber, task);
  }
   ...
   * increments the {@link #threadCount}. It also creates a corresponding lock object for the thread
   * and adds it to the {@link #locks} map. As this method is invoked in a single-threaded
   * environment (guaranteed by the {@link SchedulerThread}), there is no need to synchronize access
   * to the {@link #createdThreadList} and {@link #locks}. If the thread is already in the {@link
   * #createdThreadList}, a message is printed to the console and no further action is taken.
   *
   ...
   *
   * <p>This method is invoked when a thread in the program under test calls the start() method. The
   * thread is then added to the {@link #readyThread}. The method assigns the thread to the {@link
   * #threadStartReq} to inform the {@link SchedulerThread} that the thread is ready to run. It then
   * calls the {@link #waitRequest(Thread)} method to make the current thread wait and hand over
   * control to the {@link SchedulerThread} for deciding which thread to run next. As this method is
   * invoked in a single-threaded environment (guaranteed by the {@link SchedulerThread}), there is
   * no need to synchronize access to the {@link #readyThread}. If the thread is not in the {@link
   * #createdThreadList} or is already in the {@link #readyThread}, a message is printed to the
   ...
   * another thread. The calling thread is then set as the {@link #threadJoinReq} and the thread it
   * wishes to join is set as the {@link #threadJoinRes}. The method then calls the {@link
   ...
   *
   * <p>This method is invoked by the {@link SchedulerThread} to request permission from a thread to
   * execute. It ensures that access to {@link #threadWaitReq} is synchronized and race-free. The
   * method uses the lock associated with the thread (retrieved from the {@link #locks} map) to
   * synchronize the access. This ensures that the thread can safely run without any race
   * conditions.
   *
   ...
   * requested, or an assert statement failing. The method assigns the waiting thread to the {@link
   * #threadWaitReq} to inform the {@link SchedulerThread} that the thread is waiting. It then makes
   * the current thread wait by calling the wait() method on the lock associated with the thread
   * (retrieved from the {@link #locks} map). This hands over control to the {@link SchedulerThread}
   * for deciding which thread to run next. As this method is invoked in a single-threaded
   * environment (guaranteed by the {@link SchedulerThread}), there is no need to synchronize access
   * to the {@link #threadWaitReq}.
   *
   ...
   * thread wait by calling the wait() method on the lock associated with the main thread (retrieved
   * from the {@link #locks} map). This hands over control to the {@link SchedulerThread} for
   * deciding which thread to run next. As this method is invoked in a single-threaded environment
   ...
   * and needs to be removed from the active threads list. The thread is then removed from both the
   * {@link #createdThreadList} and {@link #readyThread}. The method checks if the thread is the
   * main thread. If it is, it sets the {@link #isFinished} flag to true, makes the main thread wait
   * by calling the {@link #waitRequest(Thread)} method, and then calls the {@link
   ...
   * execution. As this method is invoked in a single-threaded environment (guaranteed by the {@link
   * SchedulerThread}), there is no need to synchronize access to the {@link #createdThreadList},
   * {@link #readyThread}, and {@link #threadWaitReq}.
   *
   ...
   * or continue the testing with the next iteration. If the {@link #mcGraphs} list is empty and the
   * {@link #strategyType} is {@link StrategyType#TRUST}, the method terminates the program
   * execution. If the {@link #numOfExecutions} is less than the {@link #maxNumOfExecutions} and the
   * {@link #strategyType} is {@link StrategyType#RANDOM}, the method terminates the program
   * execution. If the {@link #numOfExecutions} is equal to the {@link #maxNumOfExecutions} and the
   * {@link #strategyType} is {@link StrategyType#RANDOM}, the method terminates the program
   ...
   * object. The method first creates a {@link Location} based on the provided object, owner class,
   * field name, and descriptor. It then checks if the {@link Location} is of a primitive type. If
   * it is, a {@link ReadEvent} is created for the thread and the thread are made to wait by calling
   * the {@link #waitRequest(Thread)} method. This hands over control to the {@link SchedulerThread}
   * for deciding which thread to run next. If the {@link Location} is not of a primitive type, a
   * message is printed to the console indicating that the Model Checker will not consider this
   * operation as it does not involve a primitive type. As this method is invoked in a
   * single-threaded environment (guaranteed by the {@link SchedulerThread}), there is no need to
   * synchronize access to the {@link #writeEventReq}.
   *
   ...
   * object. The method first creates a {@link Location} based on the provided object, owner class,
   * field name, and descriptor. It then checks if the {@link Location} is of a primitive type. If
   * it is, a {@link WriteEvent} is created for the thread and the thread are made to wait by
   * calling the {@link #waitRequest(Thread)} method. This hands over control to the {@link
   * SchedulerThread} for deciding which thread to run next. If the {@link Location} is not of a
   * primitive type, a message is printed to the console indicating that the Model Checker will not
   * consider this operation as it does not involve a primitive type. As this method is invoked in a
   * single-threaded environment (guaranteed by the {@link SchedulerThread}), there is no need to
   * synchronize access to the {@link #writeEventReq}.
   *
  ...
  public static void writeOperation(
      Object obj, Object newVal, Thread thread, String owner, String name, String descriptor) {
    Location location = createLocation(obj, owner, name, descriptor);
    LOGGER.debug(
  ...
  public static ReceiveEvent blockingReceiveRequestOperation(Thread thread) {
    LOGGER.debug(
        "Thread-{} requested to receive a blocking message", threadIdMap.get(thread.getId()));
   ...
   * operation. The thread is requesting to park itself. The method assigns the thread to the {@link
   * #threadToPark} to inform the {@link SchedulerThread} that the thread is going to park. It then
   ...
   *
   * <p>This method is invoked when a thread in the program under test is about to execute an unpark
   * operation. The thread is requesting to unpark another thread. The method assigns the caller
   * thread to the {@link #unparkerThread} and the callee thread to the {@link #unparkeeThread} to
   * inform the {@link SchedulerThread} that the caller thread is going to unpark the callee thread.
   * It then makes the caller thread wait by calling the {@link #waitRequest(Thread)} method. This
   * hands over control to the {@link SchedulerThread} for deciding which thread to run next.
   *
   ...
   *
   * <p>This method is invoked when a thread in the program under test is about to execute a {@code
   * MONITORENTER} operation. The thread is requesting to enter the monitor of a lock. The method
   * assigns the requesting thread to the {@link #threadEnterMonitorReq} and the lock to the {@link
   * #objectEnterMonitorReq} to inform the {@link SchedulerThread} that the thread has requested to
   * enter the monitor. It then makes the requesting thread wait by calling the {@link
   * #waitRequest(Thread)} method. This hands over control to the {@link SchedulerThread} for
   * deciding which thread to run next. As this method is invoked in a single-threaded environment
   * (guaranteed by the {@link SchedulerThread}), there is no need to synchronize access to the
   * {@link #threadEnterMonitorReq} and {@link #objectEnterMonitorReq}.
   *
   ...
   *
   * <p>This method is invoked when a thread in the program under test is about to execute a {@code
   * MONITOREXIT} operation. The thread is requesting to exit the monitor of a lock. The method logs
   * the request of the thread to exit the monitor of the lock. However, it does not perform any
   * action or change any state in the {@link RuntimeEnvironment}. The actual exit operation is
   * handled by the JVM. As this method is invoked in a single-threaded environment (guaranteed by
   * the {@link SchedulerThread}), there is no need to synchronize access to any shared resources.
   *
   ...
   * which locks, which is essential for handling synchronization and preventing deadlocks. As this
   * method is invoked in a single-threaded environment (guaranteed by the {@link SchedulerThread}),
   * there is no need to synchronize access to the {@link #monitorList}.
   *
  ...
  public static void casWithAssume(Object lock, Thread thread) throws JMCInterruptException {
    Location location =
        createLocation(
            lockAvailMap.get(lock), "org/mpisws/util/concurrent/JMCLock", "permits", "I");
  ...
  public static void addFuture(Future future, Thread thread) {
    LOGGER.debug(
        "Future ({}) has been created for thread-{}", future, threadIdMap.get(thread.getId()));
   ...
   * method is invoked in a single-threaded environment (guaranteed by the {@link SchedulerThread}),
   * there is no need to synchronize access to the {@link #assertFlag}.
   *
  ...
  public static void AssumeBlocked(Thread thread) {
    LOGGER.debug(
        "{} requested to execute a blocking assume operation", Thread.currentThread().getName());
  ...
  public static ConAssumeEvent createConAssumeEvent(Thread thread, boolean result) {
    ...
    return new ConAssumeEvent(
        threadIdMap.get(thread.getId()).intValue(), EventType.CON_ASSUME, serialNumber, result);
  }
   ...
   * test has requested to join another thread and the latter has finished its execution. The method
   * creates a {@link JoinEvent} for the requesting thread over the finished thread. The method
   ...
   *
   * <p>This method is invoked by the {@link SchedulerThread} when a thread in the program under
   * test is about to park. The method creates a {@link ParkEvent} for the thread. The method first
   * generates a serial number for the event by calling the {@link #getNextSerialNumber(Thread)}
   * method. It then creates a new {@link ParkEvent} with the {@code EventType.PARK} and the ID of
   * the thread.
   *
   ...
   * the unparkeeThread over the unparkerThread. The method first generates a serial number for the
   * event by calling the {@link #getNextSerialNumber(Thread)} method. It then creates a new {@link
   * UnparkEvent} with the {@code EventType.UNPARK}, the IDs of the unparkerThread and
   ...
   * unparkerThread. The method first generates a serial number for the event by calling the {@link
   * #getNextSerialNumber(Thread)} method. It then creates a new {@link UnparkingEvent} with the
   * {@code EventType.UNPARKING} and the ID of the unparkerThread.
   *
   ...
   * #getNextSerialNumber(Thread)} method. It then creates a new {@link FinishEvent} with the {@code
   * EventType.FINISH}, the ID of the finished thread, and the generated serial number.
   *
   ...
   *
   * <p>This method is invoked by the {@link RuntimeEnvironment} when a thread in the program under
   * test is about to execute a read operation. The method creates a {@link ReadEvent} for the
   * thread and the {@link Location} it wants to read from. The method first generates a serial
   * number for the event by calling the {@link #getNextSerialNumber(Thread)} method. It then
   * creates a new {@link ReadEvent} with the {@code EventType.READ}, the ID of the thread, the
   * generated serial number, the current value of the {@link Location}, and the {@link Location}
   * itself.
   *
   ...
   *
   * <p>This method is invoked by the {@link RuntimeEnvironment} when a thread in the program under
   * test is about to execute a write operation. The method creates a {@link WriteEvent} for the
   * thread, the {@link Location} it wants to write to, and the new value it wants to write. The
   * method first generates a serial number for the event by calling the {@link
   * #getNextSerialNumber(Thread)} method. It then creates a new {@link WriteEvent} with the {@code
   * EventType.WRITE}, the ID of the thread, the generated serial number, the new value, and the
   * {@link Location} itself.
   *
   ...
   *
   * <p>This method is invoked by the {@link RuntimeEnvironment} when a thread in the program under
   * test is about to execute a {@code MONITORENTER} operation. The method creates an {@link
   * EnterMonitorEvent} for the thread and the lock it wants to enter. The method first generates a
   * serial number for the event by calling the {@link #getNextSerialNumber(Thread)} method. It then
   * creates a new {@link EnterMonitorEvent} with the {@code EventType.ENTER_MONITOR}, the ID of the
   ...
   *
   * <p>This method is invoked by the {@link RuntimeEnvironment} when a thread in the program under
   * test is about to execute a {@code MONITOREXIT} operation. The method creates an {@link
   * ExitMonitorEvent} for the thread and the lock it wants to exit. The method first generates a
   * serial number for the event by calling the {@link #getNextSerialNumber(Thread)} method. It then
   * creates a new {@link ExitMonitorEvent} with the {@code EventType.EXIT_MONITOR}, the ID of the
   ...
   *
   * <p>This method is invoked by the {@link SchedulerThread} when it decides to suspend a thread in
   * the program under, based on its strategy. The method creates a {@link SuspendEvent} for the
   * thread and the monitor the thread wants to acquire. The method first generates a serial number
   * for the event by calling the {@link #getNextSerialNumber(Thread)} method. It then creates a new
   * {@link SuspendEvent} with the {@code EventType.SUSPEND}, the ID of the thread, the generated
   * serial number, and the monitor it wants to suspend.
   *
   ...
   *
   * <p>This method is invoked by the {@link RuntimeEnvironment} when a thread in the program under
   * test is about to execute a symbolic execution. The method creates a {@link SymExecutionEvent}
   * for the thread and the formula it wants to execute symbolically. The method first generates a
   * serial number for the event by calling the {@link #getNextSerialNumber(Thread)} method. It then
   * creates a new {@link SymExecutionEvent} with the {@code EventType.SYM_EXECUTION}, the ID of the
   * thread, the generated serial number, the result of the solver, the formula to execute
   * symbolically, and whether the formula is negatable.
   *
   ...
   *
   * <p>This method is invoked by the {@link RuntimeEnvironment} when a thread in the program under
   * test is about to execute a {@code MONITORENTER} operation. The method creates a {@link
   * MonitorRequestEvent} for the thread and the lock it wants to enter. The method first generates
   * a serial number for the event by calling the {@link #getNextSerialNumber(Thread)} method. It
   * then creates a new {@link MonitorRequestEvent} with the {@code EventType.MONITOR_REQUEST}, the
   * ID of the thread, the generated serial number, and the monitor it wants to enter.
   *
   ...
   *
   * <p>This method is invoked by the {@link RuntimeEnvironment} when a thread in the program under
   * test is about to execute a {@code MONITORENTER} or {@code MONITOREXIT} operation. The method
   * creates a {@link Monitor} object for the lock that the thread wants to enter or exit. The
   * method first retrieves the Class object of the lock and then creates a new {@link Monitor} with
   * the Class object and the lock. The created {@link Monitor} object is then used to create an
   * {@link EnterMonitorEvent} or {@link ExitMonitorEvent} for the thread. As this method is invoked
   * in a single-threaded environment (guaranteed by the {@link SchedulerThread}), there is no need
   * to synchronize access to the lock.
   *
  ...
  /**
   * Retrieves the {@link StaticMethodMonitor} for a synchronized static method that is about to be
   * executed.
   *
   * <p>This method is invoked by the {@link RuntimeEnvironment} when a thread in the program under
   * test is about to execute a synchronized static method. The method retrieves the {@link
   ...
   * method is not found in the list, a new {@link StaticMethodMonitor} is created and added to the
   * list.
   *
  ...
  /**
   * Retrieves the {@link InstanceMethodMonitor} for a synchronized instance method that is about to
   * be executed.
   *
   * <p>This method is invoked by the {@link RuntimeEnvironment} when a thread in the program under
   * test is about to execute a synchronized instance method. The method retrieves the {@link
   * InstanceMethodMonitor} for the method from the {@link #instanceMethodMonitorList} based on the
   * object(this), method name, and method descriptor. If the {@link InstanceMethodMonitor} for the
   * method is not found in the list, a new {@link InstanceMethodMonitor} is created and added to
   * the list.
   *
   ...
   *
   * <p>This method is invoked by the {@link RuntimeEnvironment} when a thread in the program under
   * test is about to execute a read or write operation. The method creates a {@link Location}
   * object for the field that the thread wants to access. The method first retrieves the Class
   * object of the owner class and the Field object of the field. It then retrieves the current
   * value of the field from the provided object. Using these, it creates a new {@link Location}
   * object with the Class object, the object instance, the Field object, the current value, and the
   * descriptor of the field. The created {@link Location} object can then be used to create a
   * {@link ReadEvent} or {@link WriteEvent} for the thread. As this method is invoked in a
   * single-threaded environment (guaranteed by the {@link SchedulerThread}), there is no need to
   ...
   * #mcThreadSerialNumber} map, increments it by one, and then updates the map with the new serial
   * number. The serial number is used to order the events of a thread in the sequence they were
   * created. This allows the {@link SchedulerThread} to replay the events in the same order during
   * the model checking process. As this method is invoked in a single-threaded environment
   * (guaranteed by the {@link SchedulerThread}), there is no need to synchronize access to the
   ...
   * model checking process should terminate and a {@link FinishedType} field indicating the type of
   * the finish event. It is serialized and written to a file named "finish.obj" in the
   ...
   * initial values, effectively preparing the {@link RuntimeEnvironment} for the next execution of
   * the program.
   */
