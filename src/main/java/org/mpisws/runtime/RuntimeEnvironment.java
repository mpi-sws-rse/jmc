...
public class RuntimeEnvironment {
    ...
    public static void receiveOperation(long receiverId) {
    ...
    public static void receiveBlockOperation(long receiverId) {
        Thread thread = Thread.currentThread();
        System.out.println(
                "[Runtime Environment Message] : Thread-" + threadIdMap.get(thread.getId()) + " requested to " +
                        "receive a message"
        );
        receiveEventReq = createReceiveBlockEvent(thread);
        waitRequest(thread);
    }

    public static void receiveTaggedBlockOperation(long receiverId, BiFunction<Long, Long, Boolean> function) {
        Thread thread = Thread.currentThread();
        System.out.println(
                "[Runtime Environment Message] : Thread-" + threadIdMap.get(thread.getId()) + " requested to " +
                        "receive a message"
        );
        receiveEventReq = createReceiveTaggedBlockEvent(thread, function);
        waitRequest(thread);
    }

    public static void receiveTaggedUnblockOperation(long receiverId, BiFunction<Long, Long, Boolean> function) {
        Thread thread = Thread.currentThread();
        System.out.println(
                "[Runtime Environment Message] : Thread-" + threadIdMap.get(thread.getId()) + " requested to " +
                        "receive a message"
    ...
    public static Message sendTaggedMessageOperation(Thread thread, long receiverId, int tag, Object value) {
        Message message = createTaggedMessage(receiverId, Thread.currentThread().getId(), value, tag);
        System.out.println(
                "[Runtime Environment Message] : Thread-" + threadIdMap.get(thread.getId()) + " requested to " +
                        "send a tagged message to Thread-" + receiverId
        );
