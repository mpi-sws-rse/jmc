...
public class RandomCounter extends Thread {
  ...
  public RandomCounter(
      Counter counter, SymbolicInteger x, SymbolicInteger y, SymbolicBoolean a, SymbolicBoolean b) {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    try {
      assert (counter.getValue() == 2)
          : " ***The assert did not pass, the counter value is " + counter.getValue() + "***";
    } catch (AssertionError e) {
      System.out.println(e.getMessage());
    }

    System.out.println(
        "[Random Counter message] : If you see this message, the assert passed. The counter value"
            + " is "
            + counter.getValue());
  }
}
